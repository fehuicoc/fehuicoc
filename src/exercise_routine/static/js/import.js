/**
 * MOD-ER-IMPORT — upload, preview, cancel, confirm → library upsert.
 */
(function () {
  const PREVIEW_KEY = "er_import_preview_v1";
  const MAX_BYTES =
    typeof window.ER_MAX_FILE_BYTES === "number"
      ? window.ER_MAX_FILE_BYTES
      : 1048576;
  ...
  if (!fileInput || !window.ERLibrary) return;
  ...
  function renderPreview(data) {
    ...
    sessions.forEach((s) => {
      ...
      const exNames = (s.exercises || [])
        ...
        .join("; ");
      detail.textContent =
        (s.exercise_count || (s.exercises || []).length) +
        " exercise(s): " +
        (exNames || "—");
  ...
  async function validateFile(file) {
    ...
    try {
      ...
      validating.hidden = true;
      const msg =
        "Could not reach import preview API. Library was not changed.";
      showErrors([msg]);
      setStatus(msg, "error");
      return;
    }
    ...
    try {
      ...
      showErrors(["Invalid response from import preview."]);
      setStatus("Import failed", "error");
      return;
    }
    if (!res.ok || !data.ok) {
      showErrors(data.errors || ["Validation failed."]);
  ...
  function confirmImport() {
    ...
    if (btnEditAfter) {
      btnEditAfter.href = "/author?id=" + encodeURIComponent(toSave.id);
